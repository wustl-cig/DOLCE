...
#
# Livermore Enhanced AI Projector for tomographic reconstruction (LEAP)
################################################################################
...
class ProjectorFunctionCPU(torch.autograd.Function):
    ...
    def forward(ctx, input, proj, vol, param_id): # input: image, output: projection (sinogram)
        for batch in range(input.shape[0]):
            ...
            g = proj[batch]
            leap.project_cpu(param_id, g, f_xyz) # compute proj (g) from input (f)
            proj[batch] = g
    ...
    def backward(ctx, grad_output): # grad_output: projection (sinogram) grad_input: image
        ...
        for batch in range(input.shape[0]):
            ...
            f_xyz = torch.Tensor.contiguous(torch.permute(f, (2, 1, 0)))
            g = grad_output[batch]
            leap.backproject_cpu(param_id, g, f_xyz) # compute input (f) from proj (g)
...
class ProjectorFunctionGPU(torch.autograd.Function):
    ...
    def forward(ctx, input, proj, vol, param_id): # input: image, output: projection (sinogram)
        for batch in range(input.shape[0]):
            ...
            f_xyz = torch.Tensor.contiguous(torch.permute(f, (2, 1, 0)))
            g = proj[batch]
            leap.project_gpu(param_id, g, f_xyz) # compute proj (g) from input (f)
            proj[batch] = g
        ctx.save_for_backward(input, proj, vol, param_id)
        return proj
    ...
    def backward(ctx, grad_output): # grad_output: projection (sinogram) grad_input: image
        ...
        for batch in range(input.shape[0]):
            ...
            f_xyz = torch.Tensor.contiguous(torch.permute(f, (2, 1, 0)))
            g = grad_output[batch]
            leap.backproject_gpu(param_id, g, f_xyz) # compute input (f) from proj (g)
            f_zyx = torch.Tensor.contiguous(torch.permute(f_xyz, (2, 1, 0)))
            vol[batch] = f_zyx
        return vol, None, None, None
...
class Projector(torch.nn.Module):
    def __init__(self, forward_project=True, use_static=False, use_gpu=False, gpu_device=None, gpu_device_id=0, batch_size=1):
        super(Projector, self).__init__()
        self.forward_project = forward_project
        self.param_id = -1 if use_static else leap.create_param()
        self.param_id_t = torch.tensor(self.param_id).to(gpu_device) if use_gpu else torch.tensor(self.param_id)
        self.use_gpu = use_gpu
        self.gpu_device = gpu_device
        self.gpu_device_id = gpu_device_id
        leap.set_gpu(self.param_id, self.gpu_device_id)
    ...
    def set_volume(self, dimx, dimy, dimz, width, height, offsetx, offsety, offsetz):
        leap.set_volume(self.param_id, dimx, dimy, dimz, width, height, offsetx, offsety, offsetz)
    ...
    def set_parallel_beam(self, nangles, nrows, ncols, pheight, pwidth, crow, ccol, arange, phis):
        leap.set_parallel_beam(self.param_id, nangles, nrows, ncols, pheight, pwidth, crow, ccol, arange, phis)
    ...
    def set_cone_beam(self, nangles, nrows, ncols, pheight, pwidth, crow, ccol, arange, phis, sod, sdd):
        leap.set_cone_beam(self.param_id, nangles, nrows, ncols, pheight, pwidth, crow, ccol, arange, phis, sod, sdd)
        proj_np = np.zeros((self.batch_size, nangles, nrows, ncols)).astype(np.float32)
        self.proj_data = torch.from_numpy(proj_np)
        if self.use_gpu:
            self.proj_data = self.proj_data.float().to(self.gpu_device)
    ...
    def set_modular_beam(self, nangles, nrows, ncols, pheight, pwidth, srcpos, modcenter, rowvec, colvec):
        leap.set_modular_beam(self.param_id, nangles, nrows, ncols, pheight, pwidth, srcpos, modcenter, rowvec, colvec)
        proj_np = np.zeros((self.batch_size, nangles, nrows, ncols)).astype(np.float32)
        self.proj_data = torch.from_numpy(proj_np)
        if self.use_gpu:
            self.proj_data = self.proj_data.float().to(self.gpu_device)
    ...
    def get_volume_dim(self):
        ...
        leap.get_volume_dim(self.param_id, dim_tensor)
        dim = dim_tensor.cpu().detach().numpy()
        return dim[2], dim[1], dim[0]

    def get_projection_dim(self):
        dim_tensor = torch.from_numpy(np.array([0,0,0]).astype(np.int32))
        leap.get_projection_dim(self.param_id, dim_tensor)
        dim = dim_tensor.cpu().detach().numpy()
    ...
    def save_param(self, param_fn):
        return leap.save_param(self.param_id, param_fn)

    def set_symmetry_axis(self, val):
        return leap.set_symmetry_axis(self.param_id, val)
        
    def set_projector(self, which):
        return leap.set_projector(self.param_id, which)

    def set_gpu(self, which):
        self.gpu_device_id = which
        return leap.set_gpu(self.param_id, self.gpu_device_id)
        
    def print_param(self):
        leap.print_param(self.param_id)
